...
If you want to try this on actual hardware, the process is similar but you use the RPi firmware boot chain instead of UEFI.
...
curl -LO https://arch-linux-repo.drzee.net/arch/tarballs/os/aarch64/archlinux-bootstrap-2026.03.15-aarch64.tar.zst
tar xf archlinux-bootstrap-2026.03.15-aarch64.tar.zst -C /mnt --strip-components=1
```
...
pacman -Sy
pacman -S base linux-rpi5 linux-firmware-rpi5 rpi5-eeprom raspberrypi-utils

passwd  # seriously, don't forget this
```

### Grab the boot firmware manually

This is the annoying part — `start4.elf` and friends aren't in any package:

```bash
cd /boot
for f in start4.elf fixup4.dat bcm2712-rpi-5-b.dtb bcm2712d0-rpi-5-b.dtb; do
  curl -LO "https://github.com/raspberrypi/firmware/raw/master/boot/$f"
done
```
...
echo "root=UUID=$(blkid -s UUID -o value /dev/sdX2) rw rootwait console=tty1" > /boot/cmdline.txt
```
...
Stick the drive in your Pi and power it on.
