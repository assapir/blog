...
That flag isn't free, and you can measure it. The whole program is `^ = () -> Num => 42`, one line, and it builds to a **6.6 MB** executable. A Rust staticlib carries the standard library with it, so the runtime archive is 23 MB before anything of mine is in it, and `--whole-archive` says take all of it rather than only the parts referenced. `strip` gets the binary down to 1.2 MB, which means most of what's left is symbols. For scale, the compiler itself is 1.9 MB. One line of Quilon produces an artifact three times the size of the thing that compiled it.
